...
import { Ht1Pg48Data } from './ht1_pg48.data';

@IonicPage()
@Component({
  selector: 'ht1-page-pg48',
  templateUrl: 'ht1_pg48.html',
})
export class Ht1Pg48Page {

	/* Common */
	pgConfig = new Ht1PgConfig(48);
	end_zone_num = 5;
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput27) {
			this.showUcInput27();
		}
	...
	selectUcInput10(num) {
		...
		alert.addInput({
			type: 'radio',
			label: this.dtPg.textUcInput1[1],
			value: '1',
			checked: false
		});
		...
		alert.addInput({
			type: 'radio',
			label: this.dtPg.textUcInput1[2],
			value: '2',
			checked: false
		});
		...
		alert.addInput({
			type: 'radio',
			label: this.dtPg.textUcInput1[3],
			value: '3',
			checked: false
		});
		...
		alert.addButton({
			text: '선택',
			handler: data => {
				this.dtPg.resultUcInput10[num] = Number(data);
			}
		});
	...
	selectUcInput27(num) {
		...
		alert.addInput({
			type: 'radio',
			label: this.dtPg.textUcInput2[1],
			value: '1',
			checked: false
		});
		...
		alert.addInput({
			type: 'radio',
			label: this.dtPg.textUcInput2[2],
			value: '2',
			checked: false
		});
		...
		alert.addInput({
			type: 'radio',
			label: this.dtPg.textUcInput2[3],
			value: '3',
			checked: false
		});
		...
		alert.addButton({
			text: '선택',
			handler: data => {
				this.dtPg.resultUcInput27[num] = Number(data);
			}
		});
	...
	sendUcInput10()
	{
			...
			this.dtPg.resultUcInput10[3] == -1)
		{
			this.util.toast('보기의 항목을 모두 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput10[1] == this.dtPg.passUcInput10[1] &&
			this.dtPg.resultUcInput10[2] == this.dtPg.passUcInput10[2] &&
			this.dtPg.resultUcInput10[3] == this.dtPg.passUcInput10[3])
		{
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput10 = true;
		} else {

			this.dtPg.countUcInput10--;

			if (this.dtPg.countUcInput10 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput10 = true;
				this.dtPg.resultUcInput10 = this.dtPg.passUcInput10;
			}
	...
	sendUcInput27()
	{
			...
			this.dtPg.resultUcInput27[3] == -1)
		{
			this.util.toast('보기의 항목을 모두 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput27[1] == this.dtPg.passUcInput27[1] &&
			this.dtPg.resultUcInput27[2] == this.dtPg.passUcInput27[2] &&
			this.dtPg.resultUcInput27[3] == this.dtPg.passUcInput27[3])
		{
			this.util.alert('잘 하셨습니다.');
