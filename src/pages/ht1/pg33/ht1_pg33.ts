...
import { Ht1Pg33Data } from './ht1_pg33.data';

@IonicPage()
@Component({
  selector: 'ht1-page-pg33',
  templateUrl: 'ht1_pg33.html',
})
export class Ht1Pg33Page {

	/* Common */
	pgConfig = new Ht1PgConfig(33);
	...
	checkInput()
	{
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating20) {
			if (this.dtPg.resultUcFeelRating20 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating20 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
	...
	showPage()
	{
		...
		} else if (this.item.ht_userdata == this.dtPg.sceneUcFeelRating20) {
			this.showUcFeelRating20();
	...
	showUcFeelRating20()
	{
		console.log('showUcFeelRating20()');
		return;
	}
	...
	sendUcFeelRating20()
	{
		console.log("sendUcFeelRating20", this.dtPg.resultUcFeelRating20);
		...
		if (this.dtPg.resultUcFeelRating20 == -1)
		{
			...
			this.util.toast('기분평점을 평가하셨습니다.');
			this.dtPg.isUcFeelRating20 = true;
			return;		
		}
	}
