...
import { Ht1Pg19Data } from './ht1_pg19.data';
import { Ht1Pg14Data } from '../pg14/ht1_pg14.data';

@IonicPage()
@Component({
  selector: 'ht1-page-pg19',
  templateUrl: 'ht1_pg19.html',
})
export class Ht1Pg19Page {

	/* Common */
	pgConfig = new Ht1PgConfig(19);
	...
	checkInput()
	{
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating14) {
			if (this.dtPg.countUcFeelRating14 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
	...
	showPage()
	{
		...
		/* 디스플레이처리 파트 */
		if (this.item.ht_userdata == this.dtPg.sceneUcAudio9) {
		...
		} else if (this.item.ht_userdata == this.dtPg.sceneUcAudio11) {
			this.showUcAudio(this.item.ht_base_audio, this.item.ht_text2);
			...
			this.showUcAudio(this.item.ht_base_audio, this.item.ht_text2);
		} else if (this.item.ht_userdata == this.dtPg.sceneUcFeelRating14) {
			this.showUcFeelRating14();
	...
	showUcFeelRating14()
	{
		console.log('showUcFeelRating14()');
	...
	sendUcFeelRating14()
	{
		console.log("sendUcFeelRating14", this.dtPg.countUcFeelRating14);
