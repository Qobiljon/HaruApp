...
import { Ht1Pg25Data } from './ht1_pg25.data';

@IonicPage()
@Component({
  selector: 'ht1-page-pg25',
  templateUrl: 'ht1_pg25.html',
})
export class Ht1Pg25Page {

	/* Common */
	pgConfig = new Ht1PgConfig(25);
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg25Page');
	}
	...
	checkInput()
	{
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating27) {
			if (this.dtPg.resultUcFeelRating27 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating27 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput18) {
			if (this.dtPg.resultUcInput18 == -1)
			{
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcFeelRating16) {
			this.showUcFeelRating16();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcFeelRating27) {
			this.showUcFeelRating27();
	...
	showUcFeelRating16()
	{
		console.log('showUcFeelRating16()');
		return;
	}

	showUcFeelRating27()
	{
		console.log('showUcFeelRating27()');
		return;
	}
	...
	sendUcFeelRating27()
	{
		console.log("sendUcFeelRating27", this.dtPg.resultUcFeelRating27);
		...
		if (this.dtPg.resultUcFeelRating27 == -1)
		{
			...
			this.util.toast('기분평점을 평가하셨습니다.');
			this.dtPg.isUcFeelRating27 = true;
			return;		
		}
	}
	...
	sendUcInput18()
	{
		...
		if (this.dtPg.resultUcInput18 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput18 == this.dtPg.passUcInput18)
		{
	...
	sendUcInput20()
	{
		...
		if (this.dtPg.resultUcInput20 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput20 == this.dtPg.passUcInput20)
		{
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput20 = true;
		} else {

			this.dtPg.countUcInput20--;

			if (this.dtPg.countUcInput20 >= 0) {
				this.util.alert('다시 한 번 생각해보세요. 다시 한번 생각해 보세요.');
			} else {
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput20 = true;
				this.dtPg.resultUcInput20 = this.dtPg.passUcInput20;
			}
	...
	sendUcInput25()
	{
		...
		if (this.dtPg.resultUcInput25 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput25 == this.dtPg.passUcInput25[1] ||
				this.dtPg.resultUcInput25 == this.dtPg.passUcInput25[5])
		{
			this.util.alert('잘 하셨습니다.');
			...
			if (this.dtPg.countUcInput25 >= 0) {
				...
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput25 = true;
				this.dtPg.resultUcInput25 = this.dtPg.passUcInput25[1];
			}
