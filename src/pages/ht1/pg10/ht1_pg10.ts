...
import { Ht1Pg10Data } from './ht1_pg10.data';
import { Ht1Pg07Data } from '../pg07/ht1_pg07.data';
import { Ht1Pg08Data } from '../pg08/ht1_pg08.data';

@IonicPage()
@Component({
  selector: 'ht1-page-pg10',
  templateUrl: 'ht1_pg10.html',
})
export class Ht1Pg10Page {

	/* Common */
	pgConfig = new Ht1PgConfig(10);
	...
	dtPg2nd = new Ht1Pg08Data();
	dtPg = new Ht1Pg10Data();
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg10Page');
	}
	...
	loadUcData()
	{
		...
		let dtPg2nd = new Ht1Pg08Data();
		dtPg2nd = this.storage.getObject('dtPg08');
		...
		if (this.pgConfig.isDebugClearData)
		{
			// 사용자데이터 삭제
			this.storage.remove(this.pgConfig.namePgClassCurrent);		
		}

		let dtPgData = new Ht1Pg10Data();
	...
	goPrev()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData21 && this.dtPg.selectUcData17 == 1) {
			// 분기#1
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData18;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData18 && this.dtPg.selectUcData17 == 1) {
			// 분기#1
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData17;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData21 && this.dtPg.selectUcData17 == 2) {
			// 분기#2
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData19;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData19 && this.dtPg.selectUcData17 == 2) {
			// 분기#2
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData17;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData21 && this.dtPg.selectUcData17 == 3) {
			// 분기#3
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData20;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData20 && this.dtPg.selectUcData17 == 3) {
			// 분기#3
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData17;
		} else {
	...
	goNext()
	{
		...
		if (this.checkInput() == true)
		{
			...
			if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData17 && this.dtPg.selectUcData17 == 1) {
				// 분기#1
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData18;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData18 && this.dtPg.selectUcData17 == 1) {
				// 분기#1
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData21;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData17 && this.dtPg.selectUcData17 == 2) {
				// 분기#2
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData19;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData19 && this.dtPg.selectUcData17 == 2) {
				// 분기#2
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData21;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData17 && this.dtPg.selectUcData17 == 3) {
				// 분기#3
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData20;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData20 && this.dtPg.selectUcData17 == 3) {
				// 분기#3
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcData17) {
		...
		} else if (this.item.ht_userdata == this.dtPg.sceneUcData18 ||
			this.item.ht_userdata == this.dtPg.sceneUcData19 ||
			this.item.ht_userdata == this.dtPg.sceneUcData20 ) {
	...
	sortTimeChart()
	{
		...
		let result = [
			...
			{ rank: 6, point: this.dtPg2nd.resultUcFeelRating1, input:this.dtPg2nd.resultUcInput1},
			{ rank: 7, point: this.dtPg2nd.resultUcFeelRating2, input:this.dtPg2nd.resultUcInput2},
			{ rank: 8, point: this.dtPg2nd.resultUcFeelRating3, input:this.dtPg2nd.resultUcInput3},
			{ rank: 9, point: this.dtPg2nd.resultUcFeelRating4, input:this.dtPg2nd.resultUcInput4},
			{ rank: 10, point: this.dtPg2nd.resultUcFeelRating5, input:this.dtPg2nd.resultUcInput5},
			{ rank: 11, point: this.dtPg.resultUcFeelRating1, input:this.dtPg.resultUcInput1},
			{ rank: 12, point: this.dtPg.resultUcFeelRating2, input:this.dtPg.resultUcInput2},
			{ rank: 13, point: this.dtPg.resultUcFeelRating3, input:this.dtPg.resultUcInput3},
			{ rank: 14, point: this.dtPg.resultUcFeelRating4, input:this.dtPg.resultUcInput4},
			{ rank: 15, point: this.dtPg.resultUcFeelRating5, input:this.dtPg.resultUcInput5}
		];
		...
		console.log('result[0].point',  result[0].point);
		console.log('result[14].point',  result[14].point);

		if ((result[0].point - result[14].point) >= 3) {
			this.dtPg.selectUcData17 = 1;
		} else if (result[0].point - result[14].point > 0 && result[0].point - result[14].point < 3) {
			this.dtPg.selectUcData17 = 2;
		} else {
			this.dtPg.selectUcData17 = 3;
		}

		console.log('result[0].point - result[14].point',  result[0].point - result[14].point);
		console.log('this.dtPg.selectUcData17',  this.dtPg.selectUcData17);

		this.dtPg.resultUcData17 = result;
	...
	}	drawTimeChart()
	{
		...
		if (this.item.ht_tpcode == 'hcst017') {
			setTimeout(() => {
	...
	drawTimeChart2()
	{
		...
		this.pgConfig.objLineChart = new Chart(this.lineCanvas.nativeElement, {
			...
			data: {
				...
				this.dtPg2nd.resultUcAlarm1['ampm']+' '+this.dtPg2nd.resultUcAlarm1['hour']+'시', this.dtPg2nd.resultUcAlarm2['ampm']+' '+this.dtPg2nd.resultUcAlarm2['hour']+'시', this.dtPg2nd.resultUcAlarm3['ampm']+' '+this.dtPg2nd.resultUcAlarm3['hour']+'시', this.dtPg2nd.resultUcAlarm4['ampm']+' '+this.dtPg2nd.resultUcAlarm4['hour']+'시', this.dtPg2nd.resultUcAlarm5['ampm']+' '+this.dtPg2nd.resultUcAlarm5['hour']+'시',
				...
				datasets: [{
					...
					this.dtPg2nd.resultUcFeelRating1, this.dtPg2nd.resultUcFeelRating2, this.dtPg2nd.resultUcFeelRating3, this.dtPg2nd.resultUcFeelRating4, this.dtPg2nd.resultUcFeelRating5,
		...
		if (this.item.ht_tpcode == 'hcst018' || this.item.ht_tpcode == 'hcst019' || this.item.ht_tpcode == 'hcst020') {
			setTimeout(() => {
