...
import { Ht1Pg37Data } from './ht1_pg37.data';

@IonicPage()
@Component({
  selector: 'ht1-page-pg37',
  templateUrl: 'ht1_pg37.html',
})
export class Ht1Pg37Page {

	/* Common */
	pgConfig = new Ht1PgConfig(37);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput14) {
			if (this.dtPg.resultUcInput14[1] == -1 ||
			...
			if (this.dtPg.resultUcInput19[1] == -1 ||
				this.dtPg.resultUcInput19[2] == -1)
			{
				this.util.toast('보기의 개수를 모두 선택하세요.');
				return false;
			...
			if (this.dtPg.resultUcInput22[1] == -1 ||
			...
			if (this.dtPg.resultUcInput26[1] == -1 ||
				this.dtPg.resultUcInput26[2] == -1)
			{
				this.util.toast('보기의 항목을 모두 선택하세요.');
				return false;
	...
	selectUcInput19(num) {
		...
		alert.addButton({
			text: '선택',
			handler: data => {
				this.dtPg.resultUcInput19[num] = Number(data);
			}
		});
	...
	changeUp(num) {
		...
		if (this.dtPg.resultUcInput22[num] < 10)
		{
	...
	sendUcInput14()
	{
			...
			this.dtPg.resultUcInput14[6] == -1)
		{
			this.util.toast('보기의 항목을 모두 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput14[1] == this.dtPg.passUcInput14[1] &&
			this.dtPg.resultUcInput14[2] == this.dtPg.passUcInput14[2] &&
			this.dtPg.resultUcInput14[3] == this.dtPg.passUcInput14[3] &&
			this.dtPg.resultUcInput14[4] == this.dtPg.passUcInput14[4] &&
			this.dtPg.resultUcInput14[5] == this.dtPg.passUcInput14[5] &&
			this.dtPg.resultUcInput14[6] == this.dtPg.passUcInput14[6])
		{
	...
	sendUcInput22()
	{
			...
			this.dtPg.resultUcInput22[6] == -1)
		{
			...
			this.dtPg.sumUcInput22[1] = this.dtPg.resultUcInput22[1] + this.dtPg.resultUcInput22[2];
			this.dtPg.sumUcInput22[2] = this.dtPg.resultUcInput22[3] + this.dtPg.resultUcInput22[4] + this.dtPg.resultUcInput22[5] + this.dtPg.resultUcInput22[6];
