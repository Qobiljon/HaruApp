...
import { Ht1Pg04Data } from './ht1_pg04.data';

@IonicPage()
@Component({
  selector: 'ht1-page-pg04',
  templateUrl: 'ht1_pg04.html',
})
export class Ht1Pg04Page {

	/* Common */
	pgConfig = new Ht1PgConfig(4);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput21) {
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating23) {
			if (this.dtPg.resultUcFeelRating23 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating23 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcData16) {
			this.showUcData16();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcInput21) {
			this.showUcInput21();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcData22) {
			this.showUcData22();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcFeelRating23) {
			this.showUcFeelRating23();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcData24) {
			this.showUcData24();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcData27) {
			this.showUcData27();
	...
	showUcData16()
	{
		console.log('showUcData16() passed!');
		return;
	}
	...
	showUcFeelRating23()
	{
		console.log('showUcFeelRating23()');
		return;
	}
	...
	sendUcFeelRating23()
	{
		console.log("sendUcFeelRating23", this.dtPg.resultUcFeelRating23);
		...
		if (this.dtPg.resultUcFeelRating23 == -1)
		{
			...
			this.dtPg.isUcFeelRating23 = true;
			return;		
		}
	}
