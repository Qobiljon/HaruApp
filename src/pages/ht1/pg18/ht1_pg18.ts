...
import { Ht1Pg18Data } from './ht1_pg18.data';
...
@IonicPage()
@Component({
  selector: 'ht1-page-pg18',
  templateUrl: 'ht1_pg18.html',
})
export class Ht1Pg18Page {

	/* Common */
	pgConfig = new Ht1PgConfig(18);
	...
	dtPg4th = new Ht1Pg17Data();
	dtPg = new Ht1Pg18Data();
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg18Page');
	}
	...
	loadUcData()
	{
		...
		let dtPg4th = new Ht1Pg17Data();
		dtPg4th = this.storage.getObject('dtPg17');
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating7) {
			if (this.dtPg.countUcFeelRating7 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating7 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcAudio5) {
			this.showUcAudio(this.item.ht_base_audio, this.item.ht_text2);
		} else if (this.item.ht_userdata == this.dtPg.sceneUcFeelRating7) {
			this.showUcFeelRating7();
	...
	showUcData8()
	{
		console.log('showUcData8()');

		if (this.dtPg.countTodayFeelRating < this.dtPg.countUcFeelRating7) {
			this.dtPg.resultUcData8 = 1;
		} else if (this.dtPg.countTodayFeelRating == this.dtPg.countUcFeelRating7) {
			this.dtPg.resultUcData8 = 2;
		} else {
			this.dtPg.resultUcData8 = 3;
		}

		console.log('showUcData8()', this.dtPg.resultUcData8);
	}
	...
	showUcData10()
	{
		...
		let result3:number = Math.abs(this.dtPg4th.countTodayFeelRating) - Math.abs(this.dtPg4th.countUcFeelRating9);
		let result4:number = Math.abs(this.dtPg.countTodayFeelRating) - Math.abs(this.dtPg.countUcFeelRating7);
		...
		if ((Math.abs(result1) >= Math.abs(result2)) && (Math.abs(result1) >= Math.abs(result3)) && (Math.abs(result1) >= Math.abs(result4)) )
		{
			...
			// 장면 이완 오디오
			this.dtPg.resultUcData10 = 4;
		} else {
			// 근육 이완 동영상
			this.dtPg.resultUcData10 = 5;
		}
	...
	sendUcFeelRating7()
	{
		console.log("sendUcFeelRating7", this.dtPg.countUcFeelRating7);
		...
		if (this.dtPg.countUcFeelRating7 == -1)
		{
			...
			this.dtPg.isUcFeelRating7 = true;
