...
import { Ht1Pg35Data } from './ht1_pg35.data';

@IonicPage()
@Component({
  selector: 'ht1-page-pg35',
  templateUrl: 'ht1_pg35.html',
})
export class Ht1Pg35Page {

	/* Common */
	pgConfig = new Ht1PgConfig(35);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput12) {
			if (this.dtPg.countUcInput12 != 2) {
				this.util.toast('보기의 항목 2개를 선택하세요.');
				return false;
	...
	checkUcInput12(val)
	{
		this.dtPg.countUcInput12 = 0;

		if (this.dtPg.resultUcInput12[1])
			this.dtPg.countUcInput12 += 1;
		if (this.dtPg.resultUcInput12[2])
			this.dtPg.countUcInput12 += 1;
		if (this.dtPg.resultUcInput12[3])
			this.dtPg.countUcInput12 += 1;
		if (this.dtPg.resultUcInput12[4])
			this.dtPg.countUcInput12 += 1;
		if (this.dtPg.resultUcInput12[5])
			this.dtPg.countUcInput12 += 1;
		if (this.dtPg.resultUcInput12[6])
			this.dtPg.countUcInput12 += 1;
		if (this.dtPg.resultUcInput12[7])
			this.dtPg.countUcInput12 += 1;
		if (this.dtPg.resultUcInput12[8])
			this.dtPg.countUcInput12 += 1;

		if (this.dtPg.countUcInput12 > 2) {
			this.util.alert('2개를 초과할 수 없습니다.');
			val.checked = false;
		}
	...
	sendUcInput12()
	{
		...
		if (this.dtPg.countUcInput12 != 2)
		{
			...
			for (let i = 1; i <= 8; i++) {
				if (this.dtPg.resultUcInput12[i]) {
					this.dtPg.passUcInput12[1] = i;
					break;
				}
			}

			for (let i = 1; i <= 8; i++) {
				if (this.dtPg.resultUcInput12[i] && this.dtPg.passUcInput12[1] != i) {
					this.dtPg.passUcInput12[2] = i;
					break;
				}
			}
