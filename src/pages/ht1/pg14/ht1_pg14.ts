...
import { Ht1Pg14Data } from './ht1_pg14.data';

@IonicPage()
@Component({
  selector: 'ht1-page-pg14',
  templateUrl: 'ht1_pg14.html',
})
export class Ht1Pg14Page {

	/* Common */
	pgConfig = new Ht1PgConfig(14);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcAlarm25) {
			if (this.dtPg.isUcAlarm25 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcAlarm25) {
			this.showUcAlarm25();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcInput16) {
			this.showUcInput16();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcInput26) {
			this.showUcInput26();
	...
	sendUcAlarm25()
	{
		console.log("sendUcAlarm25");

		if (this.dtPg.isUcAlarm25 == false) {
			this.util.toast('훈련 시간을 입력하셨습니다.');
			this.dtPg.isUcAlarm25 = true;

			console.log("dtPg.resultUcAlarm25.ampm", this.dtPg.resultUcAlarm25.ampm);
			console.log("dtPg.resultUcAlarm25.hour", this.dtPg.resultUcAlarm25.hour);
	...
	changeUcAmPmUp()
	{
		if (this.dtPg.resultUcAlarm25.ampm == '오전') {
			this.dtPg.resultUcAlarm25.ampm = '오후';
		} else {
			this.dtPg.resultUcAlarm25.ampm = '오전';
		}
	}
	...
	changeUcHourUp()
	{
		if (this.dtPg.resultUcAlarm25.hour == 12) {
			this.dtPg.resultUcAlarm25.hour = 1;
		} else {
			this.dtPg.resultUcAlarm25.hour += 1;
		}
	}
	...
	changeUcHourDown()
	{
		if (this.dtPg.resultUcAlarm25.hour == 1) {
			this.dtPg.resultUcAlarm25.hour = 12;
		} else {
			this.dtPg.resultUcAlarm25.hour -= 1;
		}
	}
	...
	changeUcMinUp()
	{
		if (this.dtPg.resultUcAlarm25.min == 55) {
			this.dtPg.resultUcAlarm25.min = 0;
		} else {
			this.dtPg.resultUcAlarm25.min += 5;
		}
	}

	changeUcMinDown()
	{
		if (this.dtPg.resultUcAlarm25.min == 0) {
			this.dtPg.resultUcAlarm25.min = 55;
		} else {
			this.dtPg.resultUcAlarm25.min -= 5;
		}
	}
