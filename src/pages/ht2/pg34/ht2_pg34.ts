...
import { Ht2Pg34Data } from './ht2_pg34.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg34',
  templateUrl: 'ht2_pg34.html',
})
export class Ht2Pg34Page {

	/* Common */
	pgConfig = new Ht2PgConfig(34);
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg34Page');
	}
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating16) {
			...
			if (this.dtPg.resultUcInput23[1] == false &&
				this.dtPg.resultUcInput23[2] == false &&
				this.dtPg.resultUcInput23[3] == false &&
				this.dtPg.resultUcInput23[4] == false
			) {
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating40) {
			if (this.dtPg.countUcFeelRating40 == -1) {
				this.util.toast('이때의 기분점수를 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating40 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput44) {
			if (this.dtPg.countUcInput44 == -1) {
				this.util.toast('이때의 생각점수를 선택하세요.');
				return false;
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcData45) {
			this.showUcData45();
		}
	...
	showUcData45()
	{
		console.log('showUcData45()');

		if (this.dtPg.countUcInput20 <= 3)
		{
			this.dtPg.selectUcData45 = 1;
		} else {
			if (this.dtPg.countUcInput20 > this.dtPg.countUcInput44) {
				this.dtPg.selectUcData45 = 2;
			} else {
				this.dtPg.selectUcData45 = 3;
			}
		}
	...
	sendUcInput23()
	{
		...
		if (this.dtPg.resultUcInput23[1] == false &&
			...
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput23[1] == this.dtPg.passUcInput23[1] &&
			this.dtPg.resultUcInput23[2] == this.dtPg.passUcInput23[2] &&
			this.dtPg.resultUcInput23[3] == this.dtPg.passUcInput23[3] &&
			this.dtPg.resultUcInput23[4] == this.dtPg.passUcInput23[4]
		) {
			...
			this.dtPg.countUcInput23--;

			if (this.dtPg.countUcInput23 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput23 = true;
				this.dtPg.resultUcInput23 = this.dtPg.passUcInput23;
			}
	...
	sendUcInput39()
	{
		...
		if (this.dtPg.resultUcInput39 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput39 == this.dtPg.passUcInput39) {
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput39 = true;
			return;
		} else {
			this.dtPg.countUcInput39--;

			if (this.dtPg.countUcInput39 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput39 = true;
				this.dtPg.resultUcInput39 = this.dtPg.passUcInput39;
			}
	...
	sendUcFeelRating40()
	{
		console.log("sendUcFeelRating40", this.dtPg.countUcFeelRating40);
		...
		if (this.dtPg.countUcFeelRating40 == -1)
		{
			...
			this.util.toast('기분점수를 평가하셨습니다.');
			this.dtPg.isUcFeelRating40 = true;
	...
	sendUcInput44()
	{
		console.log("sendUcInput44", this.dtPg.countUcInput44);
