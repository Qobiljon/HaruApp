...
import { Ht2Pg01Data } from './ht2_pg01.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg01',
  templateUrl: 'ht2_pg01.html',
})
export class Ht2Pg01Page {

	/* Common */
	pgConfig = new Ht2PgConfig(1);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.pgConfig.sceneTodaySleepRating) {
			if (this.pgConfig.isTodaySleepRating == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcSleepTime[1] && !this.pgConfig.isTodaySleepRating) {
