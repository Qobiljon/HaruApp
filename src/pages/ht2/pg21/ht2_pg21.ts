...
import { Ht2Pg21Data } from './ht2_pg21.data';
import { Ht2Pg16Data } from '../pg16/ht2_pg16.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg21',
  templateUrl: 'ht2_pg21.html',
})
export class Ht2Pg21Page {

	/* Common */
	pgConfig = new Ht2PgConfig(21);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneTodayFeelRating) {
			...
			if (this.dtPg.countUcFeelRating13 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating15) {
			if (this.dtPg.countUcFeelRating15 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating15 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating17) {
	...
	showPage()
	{
		...
		} else if (this.item.ht_userdata == this.dtPg.sceneUcAudio14) {
			this.showUcAudio(this.item.ht_base_audio, this.item.ht_text2);
		} else if (this.item.ht_userdata == this.dtPg.sceneUcFeelRating15) {
			this.showUcFeelRating15();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcAudio16) {
			this.showUcAudio(this.item.ht_base_audio, this.item.ht_text2);
		} else if (this.item.ht_userdata == this.dtPg.sceneUcFeelRating17) {
			this.showUcFeelRating17();
	...
	showUcFeelRating15()
	{
		console.log('showUcFeelRating15()');
	...
	showUcFeelRating17()
	{
		console.log('showUcFeelRating17()');
	...
	sendUcFeelRating13()
	{
		console.log("sendUcFeelRating13", this.dtPg.countUcFeelRating13);
	...
	sendUcFeelRating15()
	{
		console.log("sendUcFeelRating15", this.dtPg.countUcFeelRating15);
		...
		if (this.dtPg.countUcFeelRating15 == -1)
		{
			...
			this.util.toast('기분평점을 평가하셨습니다.');
			this.dtPg.isUcFeelRating15 = true;
			return;		
		}
	}
