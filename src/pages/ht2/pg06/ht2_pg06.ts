...
import { Ht2Pg06Data } from './ht2_pg06.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg06',
  templateUrl: 'ht2_pg06.html',
})
export class Ht2Pg06Page {

	/* Common */
	pgConfig = new Ht2PgConfig(6);
	...
	goPrev()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData41 && this.dtPg.selectUcInput38 == 1 ) {
			// 분기#1
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData39;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData39 && this.dtPg.selectUcInput38 == 1 ) {
			// 분기#1
			this.pgConfig.sceneNumber = this.dtPg.sceneUcInput38;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData41 && this.dtPg.selectUcInput38 == 2) {
			// 분기#2
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData40;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData40 && this.dtPg.selectUcInput38 == 2) {
			// 분기#2
			this.pgConfig.sceneNumber = this.dtPg.sceneUcInput38;
		} else {
	...
	goNext()
	{
		...
		if (this.checkInput() == true)
		{
			...
			if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput38 && this.dtPg.selectUcInput38 == 1) {
				// 분기#1
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData39;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData39 && this.dtPg.selectUcInput38 == 1 ) {
				// 분기#1
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData41;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput38 && this.dtPg.selectUcInput38 == 2 ) {
				// 분기#2
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData40;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData40 && this.dtPg.selectUcInput38 == 2 ) {
				// 분기#2
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput38) {
			if (this.dtPg.isUcInput38 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		}
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput38) {
			this.showUcInput38();
		}
	...
	showUcInput38()
	{
		console.log('showUcInput38()');

		if (this.dtPg.resultUcInput38 == 1 || this.dtPg.resultUcInput38 == 3 || this.dtPg.resultUcInput38 == 5) {
			this.dtPg.selectUcInput38 = 1;
		} else if (this.dtPg.resultUcInput38 == 2 || this.dtPg.resultUcInput38 == 4) {
			this.dtPg.selectUcInput38 = 2;
		}
	...
	sendUcInput38()
	{
		...
		if (this.dtPg.resultUcInput38 == -1)
		{
