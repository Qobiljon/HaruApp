...
import { Ht2Pg45Data } from './ht2_pg45.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg45',
  templateUrl: 'ht2_pg45.html',
})
export class Ht2Pg45Page {

	/* Common */
	pgConfig = new Ht2PgConfig(45);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput15) {
			...
			if (this.dtPg.resultUcInput21[1] == false &&
				this.dtPg.resultUcInput21[2] == false &&
				this.dtPg.resultUcInput21[3] == false &&
				this.dtPg.resultUcInput21[4] == false )
			{
				this.util.toast('보기의 답을 선택하세요.');
				return false;
			...
			if (this.dtPg.resultUcInput24[1] == false &&
				this.dtPg.resultUcInput24[2] == false &&
				this.dtPg.resultUcInput24[3] == false &&
				this.dtPg.resultUcInput24[4] == false )
			{
				this.util.toast('보기의 답을 선택하세요.');
				return false;
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput15) {
			this.showUcInput15();
	...
	sendUcInput15()
	{
			...
			this.dtPg.resultUcInput15[4] == false )
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput15[1] == this.dtPg.passUcInput15[1] &&
			this.dtPg.resultUcInput15[2] == this.dtPg.passUcInput15[2] &&
			this.dtPg.resultUcInput15[3] == this.dtPg.passUcInput15[3] &&
			this.dtPg.resultUcInput15[4] == this.dtPg.passUcInput15[4] )
		{
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput15 = true;
		} else {
			this.dtPg.countUcInput15--;

			if (this.dtPg.countUcInput15 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput15 = true;
				this.dtPg.resultUcInput15 = this.dtPg.passUcInput15;
			}
	...
	sendUcInput21()
	{
			...
			this.dtPg.resultUcInput21[4] == false )
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput21[1] == this.dtPg.passUcInput21[1] &&
			this.dtPg.resultUcInput21[2] == this.dtPg.passUcInput21[2] &&
			this.dtPg.resultUcInput21[3] == this.dtPg.passUcInput21[3] &&
			this.dtPg.resultUcInput21[4] == this.dtPg.passUcInput21[4] )
		{
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput21 = true;
		} else {
			this.dtPg.countUcInput21--;

			if (this.dtPg.countUcInput21 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput21 = true;
				this.dtPg.resultUcInput21 = this.dtPg.passUcInput21;
			}
	...
	sendUcInput24()
	{
			...
			this.dtPg.resultUcInput24[4] == false )
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput24[1] == this.dtPg.passUcInput24[1] &&
			this.dtPg.resultUcInput24[2] == this.dtPg.passUcInput24[2] &&
			this.dtPg.resultUcInput24[3] == this.dtPg.passUcInput24[3] &&
			this.dtPg.resultUcInput24[4] == this.dtPg.passUcInput24[4] )
		{
			this.util.alert('잘 하셨습니다.');
