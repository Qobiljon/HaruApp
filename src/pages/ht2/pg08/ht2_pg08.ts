...
import { Ht2Pg08Data } from './ht2_pg08.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg08',
  templateUrl: 'ht2_pg08.html',
})
export class Ht2Pg08Page {

	/* Common */
	pgConfig = new Ht2PgConfig(8);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput43) {
			if (this.dtPg.resultUcInput43[1] == false &&
				this.dtPg.resultUcInput43[2] == false &&
				this.dtPg.resultUcInput43[3] == false &&
				this.dtPg.resultUcInput43[4] == false &&
				this.dtPg.resultUcInput43[5] == false &&
				this.dtPg.resultUcInput43[6] == false &&
				this.dtPg.resultUcInput43[7] == false &&
				this.dtPg.resultUcInput43[8] == false &&
				this.dtPg.resultUcInput43[9] == false &&
				this.dtPg.resultUcInput43[10] == false &&
				this.dtPg.resultUcInput43[11] == false &&
				this.dtPg.resultUcInput43[12] == false &&
				this.dtPg.resultUcInput43[13] == false)
			{
				this.util.toast('보기의 항목을 1개 이상 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput43 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput44) {
			let countCheck = 0;

			if (this.dtPg.resultUcInput43[1])
				countCheck++;
			if (this.dtPg.resultUcInput43[2])
				countCheck++;
			if (this.dtPg.resultUcInput43[3])
				countCheck++;
			if (this.dtPg.resultUcInput43[4])
				countCheck++;
			if (this.dtPg.resultUcInput43[5])
				countCheck++;
			if (this.dtPg.resultUcInput43[6])
				countCheck++;
			if (this.dtPg.resultUcInput43[7])
				countCheck++;
			if (this.dtPg.resultUcInput43[8])
				countCheck++;
			if (this.dtPg.resultUcInput43[9])
				countCheck++;
			if (this.dtPg.resultUcInput43[10])
				countCheck++;
			if (this.dtPg.resultUcInput43[11])
				countCheck++;
			if (this.dtPg.resultUcInput43[12])
				countCheck++;
			if (this.dtPg.resultUcInput43[13])
				countCheck++;

			if (this.dtPg.stepUcInput44 <= countCheck++)
			{
				this.util.toast('보기의 항목을 모두 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput44 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		}

		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput45) {
			if (this.dtPg.isUcInput45 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput43) {
			this.showUcInput43();
		}
	...
	/* 순서선택 버튼처리 */
	sendSelectOrder44(num)
	{
		this.dtPg.orderUcInput44[num] = this.dtPg.stepUcInput44;
		this.dtPg.resultUcInput44[this.dtPg.stepUcInput44] = num;
		this.dtPg.stepUcInput44++;
	}

	sendSelectClear44()
	{
		this.dtPg.orderUcInput44[1] = -1;
		this.dtPg.orderUcInput44[2] = -1;
		this.dtPg.orderUcInput44[3] = -1;
		this.dtPg.orderUcInput44[4] = -1;
		this.dtPg.orderUcInput44[5] = -1;
		this.dtPg.orderUcInput44[6] = -1;
		this.dtPg.orderUcInput44[7] = -1;
		this.dtPg.orderUcInput44[8] = -1;
		this.dtPg.orderUcInput44[9] = -1;
		this.dtPg.orderUcInput44[10] = -1;
		this.dtPg.orderUcInput44[11] = -1;
		this.dtPg.orderUcInput44[12] = -1;
		this.dtPg.orderUcInput44[13] = -1;

		this.dtPg.resultUcInput44[1] = -1;
		this.dtPg.resultUcInput44[2] = -1;
		this.dtPg.resultUcInput44[3] = -1;
		this.dtPg.resultUcInput44[4] = -1;
		this.dtPg.resultUcInput44[5] = -1;
		this.dtPg.resultUcInput44[6] = -1;
		this.dtPg.resultUcInput44[7] = -1;
		this.dtPg.resultUcInput44[8] = -1;
		this.dtPg.resultUcInput44[9] = -1;
		this.dtPg.resultUcInput44[10] = -1;
		this.dtPg.resultUcInput44[11] = -1;
		this.dtPg.resultUcInput44[12] = -1;
		this.dtPg.resultUcInput44[13] = -1;

		this.dtPg.stepUcInput44 = 1;
	}

	sendUcInput44()
	{
		console.log('sendUcInput44()');
	...
	changeUcMinUp45(num)
	{
		let count = 0;
		for (let i = 1; i <= 13; i++)
			count += this.dtPg.resultUcInput45[i];

		if (count < this.dtPg.maxUcInput45)
			this.dtPg.resultUcInput45[num] += 5;
		else
			this.util.toast('합계 시간이 30분을 초과할 수 없습니다.');

		return;
	}

	changeUcMinDown45(num)
	{

		if (this.dtPg.resultUcInput45[num] == 0)
			return;
	...
	sendUcInput45()
	{
		...
		if (count > this.dtPg.maxUcInput45)
		{
			this.util.toast('총 수면일과 시간이 30분보다 깁니다.');
			return;
		} else if (count < this.dtPg.minUcInput45) {
			this.util.toast('총 수면일과 시간이 15분보다 짧습니다.');
			return;
		} else {
			this.dtPg.isUcInput45 = true;

			this.util.alert('계획한 수면일과를 입력하였습니다.');
			return;
		}
	}
