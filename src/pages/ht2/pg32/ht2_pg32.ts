...
import { Ht2Pg32Data } from './ht2_pg32.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg32',
  templateUrl: 'ht2_pg32.html',
})
export class Ht2Pg32Page {

	/* Common */
	pgConfig = new Ht2PgConfig(32);
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg32Page');
	}
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating17) {
			if (this.dtPg.countUcFeelRating17 == -1) {
				this.util.toast('이때의 기분점수를 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating17 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput21) {
			if (this.dtPg.countUcInput21 == -1) {
			...
			if (this.dtPg.resultUcInput28 == -1) {
				this.util.toast('보기의 답을 선택하세요.');
				return false;
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating29) {
			if (this.dtPg.countUcFeelRating29 == -1) {
				this.util.toast('이때의 기분점수를 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating29 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput33) {
			if (this.dtPg.countUcInput33 == -1) {
				this.util.toast('이때의 기분점수를 선택하세요.');
				return false;
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcData34) {
			this.showUcData34();
		}
	...
	showUcData34()
	{
		console.log('showUcData34()');

		if (this.dtPg.countUcInput21 <= 3)
		{
			this.dtPg.selectUcData34 = 1;
		} else {
			if (this.dtPg.countUcInput21 > this.dtPg.countUcInput33) {
				this.dtPg.selectUcData34 = 2;
			} else {
				this.dtPg.selectUcData34 = 3;
			}
		}
	...
	sendUcFeelRating17()
	{
		console.log("sendUcFeelRating17", this.dtPg.countUcFeelRating17);
		...
		if (this.dtPg.countUcFeelRating17 == -1)
		{
			...
			this.dtPg.isUcFeelRating17 = true;
	...
	sendUcInput21()
	{
		console.log("sendUcInput21", this.dtPg.countUcInput21);
	...
	sendUcInput28()
	{
		...
		if (this.dtPg.resultUcInput28 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput28 == this.dtPg.passUcInput28) {
	...
	sendUcFeelRating29()
	{
		console.log("sendUcFeelRating29", this.dtPg.countUcFeelRating29);
		...
		if (this.dtPg.countUcFeelRating29 == -1)
		{
			...
			this.util.toast('기분점수를 평가하셨습니다.');
			this.dtPg.isUcFeelRating29 = true;
	...
	sendUcInput33()
	{
		console.log("sendUcInput33", this.dtPg.countUcInput33);
