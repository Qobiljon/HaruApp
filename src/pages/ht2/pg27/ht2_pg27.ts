...
import { Ht2Pg27Data } from './ht2_pg27.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg27',
  templateUrl: 'ht2_pg27.html',
})
export class Ht2Pg27Page {

	/* Common */
	pgConfig = new Ht2PgConfig(27);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating48) {
			if (this.dtPg.countUcFeelRating48 == -1) {
				this.util.toast('이때의 기분점수를 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating48 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating56) {
			if (this.dtPg.countUcFeelRating56 == -1) {
				this.util.toast('이때의 기분점수를 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating56 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput45) {
			if (this.dtPg.resultUcInput45 == -1) {
				this.util.toast('보기의 답을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput45 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput46) {
			if (this.dtPg.resultUcInput46 == -1) {
				this.util.toast('보기의 답을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput46 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput47) {
			if (this.dtPg.resultUcInput47 == -1) {
				this.util.toast('보기의 답을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput47 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput52) {
			if (this.dtPg.isUcInput52 == false) {
				this.util.alert('합리적인 생각찾기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput53) {
			if (this.dtPg.resultUcInput53 == -1) {
				this.util.toast('보기의 답을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput53 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput55) {
			if (this.dtPg.resultUcInput55 == -1) {
				this.util.toast('보기의 답을 선택하세요.');
				return false;
	...
	sendUcInput45()
	{
		...
		if (this.dtPg.resultUcInput45 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput45 == this.dtPg.passUcInput45) {
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput45 = true;
			return;
		} else {
			this.dtPg.countUcInput45--;

			if (this.dtPg.countUcInput45 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput45 = true;
				this.dtPg.resultUcInput45 = this.dtPg.passUcInput45;
			}
	...
	sendUcInput46()
	{
		console.log('sendUcInput46()');

		if (this.dtPg.resultUcInput46 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput46 == this.dtPg.passUcInput46) {
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput46 = true;
			return;
		} else {
			this.dtPg.countUcInput46--;

			if (this.dtPg.countUcInput46 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput46 = true;
				this.dtPg.resultUcInput46 = this.dtPg.passUcInput46;
			}
	...
	sendUcInput47()
	{
		console.log('sendUcInput47()');

		if (this.dtPg.resultUcInput47 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput47 == this.dtPg.passUcInput47) {
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput47 = true;
			return;
		} else {
			this.dtPg.countUcInput47--;

			if (this.dtPg.countUcInput47 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput47 = true;
				this.dtPg.resultUcInput47 = this.dtPg.passUcInput47;
			}
	...
	sendUcFeelRating48()
	{
		console.log("sendUcFeelRating48", this.dtPg.countUcFeelRating48);
		...
		if (this.dtPg.countUcFeelRating48 == -1)
		{
			...
			this.dtPg.isUcFeelRating48 = true;
	...
	sendUcInput53()
	{
		console.log('sendUcInput53()');

		if (this.dtPg.resultUcInput53 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput53 == this.dtPg.passUcInput53) {
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput53 = true;
			return;
		} else {
			this.dtPg.countUcInput53--;

			if (this.dtPg.countUcInput53 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput53 = true;
				this.dtPg.resultUcInput53 = this.dtPg.passUcInput53;
			}
	...
	sendUcInput55()
	{
		console.log('sendUcInput55()');

		if (this.dtPg.resultUcInput55 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput55 == this.dtPg.passUcInput55) {
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput55 = true;
			return;
		} else {
			this.dtPg.countUcInput55--;

			if (this.dtPg.countUcInput55 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput55 = true;
				this.dtPg.resultUcInput55 = this.dtPg.passUcInput55;
			}
	...
	sendUcFeelRating56()
	{
		console.log("sendUcFeelRating56", this.dtPg.countUcFeelRating56);
		...
		if (this.dtPg.countUcFeelRating56 == -1)
		{
			...
			this.util.toast('기분점수를 평가하셨습니다.');
			this.dtPg.isUcFeelRating56 = true;
