...
import { Ht2Pg47Data } from './ht2_pg47.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg47',
  templateUrl: 'ht2_pg47.html',
})
export class Ht2Pg47Page {

	/* Common */
	pgConfig = new Ht2PgConfig(47);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput13) {
			if (this.dtPg.resultUcInput13[1] == -1 &&
			...
			if (this.dtPg.isUcInput25 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
			...
			if (this.dtPg.resultUcInput13[1] != -1)
				countCheck++;
			if (this.dtPg.resultUcInput13[2] != -1)
				countCheck++;
			if (this.dtPg.resultUcInput13[3] != -1)
				countCheck++;
			if (this.dtPg.resultUcInput13[4] != -1)
				countCheck++;
			if (this.dtPg.resultUcInput13[5] != -1)
				countCheck++;
			if (this.dtPg.resultUcInput13[6] != -1)
				countCheck++;

			if (this.dtPg.stepUcInput27 <= countCheck++)
			{
	...
	sendSelectPick25(num)
	{
		if (this.dtPg.resultUcInput25[num] == -1) {
			this.dtPg.resultUcInput25[num] = num;
		} else {
			this.dtPg.resultUcInput25[num] = -1;
		}
	}
	...
	/* 순서선택 버튼처리 */
	sendSelectOrder27(num)
	{
		this.dtPg.orderUcInput27[num] = this.dtPg.stepUcInput27;
		this.dtPg.resultUcInput27[this.dtPg.stepUcInput27] = num;
		this.dtPg.stepUcInput27++;
	}

	sendSelectClear27()
	{
		this.dtPg.orderUcInput27[1] = -1;
		this.dtPg.orderUcInput27[2] = -1;
		this.dtPg.orderUcInput27[3] = -1;
		this.dtPg.orderUcInput27[4] = -1;
		this.dtPg.orderUcInput27[5] = -1;
		this.dtPg.orderUcInput27[6] = -1;
		this.dtPg.resultUcInput27[1] = -1;
		this.dtPg.resultUcInput27[2] = -1;
		this.dtPg.resultUcInput27[3] = -1;
		this.dtPg.resultUcInput27[4] = -1;
		this.dtPg.resultUcInput27[5] = -1;
		this.dtPg.resultUcInput27[6] = -1;
		this.dtPg.stepUcInput27 = 1;
	}
