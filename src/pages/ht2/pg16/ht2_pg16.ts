...
import { Ht2Pg16Data } from './ht2_pg16.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg16',
  templateUrl: 'ht2_pg16.html',
})
export class Ht2Pg16Page {

	/* Common */
	pgConfig = new Ht2PgConfig(16);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcAlarm28) {
			if (this.dtPg.isUcAlarm28 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
			...
			if (this.dtPg.resultUcInput19[1] == false &&
				...
				this.dtPg.resultUcInput19[6] == false )
			{
	...
	sendUcAlarm28()
	{
		console.log("sendUcAlarm28");

		if (this.dtPg.isUcAlarm28 == false) {
			this.util.toast('훈련 시간을 입력하셨습니다.');
			this.dtPg.isUcAlarm28 = true;

			console.log("dtPg.resultUcAlarm28.ampm", this.dtPg.resultUcAlarm28.ampm);
			console.log("dtPg.resultUcAlarm28.hour", this.dtPg.resultUcAlarm28.hour);
	...
	sendUcInput19()
	{
			...
			this.dtPg.resultUcInput19[6] == false )
		{
			...
			this.dtPg.resultUcInput19[5] == this.dtPg.passUcInput19[5] &&
			this.dtPg.resultUcInput19[6] == this.dtPg.passUcInput19[6] )
		{
	...
	changeUcAmPmUp()
	{
		if (this.dtPg.resultUcAlarm28.ampm == '오전') {
			this.dtPg.resultUcAlarm28.ampm = '오후';
		} else {
			this.dtPg.resultUcAlarm28.ampm = '오전';
		}
	}
	...
	changeUcHourUp()
	{
		if (this.dtPg.resultUcAlarm28.hour == 12) {
			this.dtPg.resultUcAlarm28.hour = 1;
		} else {
			this.dtPg.resultUcAlarm28.hour += 1;
		}
	}
	...
	changeUcHourDown()
	{
		if (this.dtPg.resultUcAlarm28.hour == 1) {
			this.dtPg.resultUcAlarm28.hour = 12;
		} else {
			this.dtPg.resultUcAlarm28.hour -= 1;
		}
	}
	...
	changeUcMinUp()
	{
		if (this.dtPg.resultUcAlarm28.min == 55) {
			this.dtPg.resultUcAlarm28.min = 0;
		} else {
			this.dtPg.resultUcAlarm28.min += 5;
		}
	}

	changeUcMinDown()
	{
		if (this.dtPg.resultUcAlarm28.min == 0) {
			this.dtPg.resultUcAlarm28.min = 55;
		} else {
			this.dtPg.resultUcAlarm28.min -= 5;
		}
	}
