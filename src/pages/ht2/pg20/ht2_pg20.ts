...
import { Ht2Pg20Data } from './ht2_pg20.data';
...
@IonicPage()
@Component({
  selector: 'ht2-page-pg20',
  templateUrl: 'ht2_pg20.html',
})
export class Ht2Pg20Page {

	/* Common */
	pgConfig = new Ht2PgConfig(20);
	...
	dtPg2nd = new Ht2Pg17Data();
	dtPg3rd = new Ht2Pg18Data();
	dtPg4th = new Ht2Pg19Data();
	dtPg = new Ht2Pg20Data();
	...
	loadUcData()
	{
		...
		let dtPg2nd = new Ht2Pg17Data();
		dtPg2nd = this.storage.getObject('dtPg17');
		...
		let dtPg3rd = new Ht2Pg18Data();
		dtPg3rd = this.storage.getObject('dtPg18');
		...
		let dtPg4th = new Ht2Pg19Data();
		dtPg4th = this.storage.getObject('dtPg19');
	...
	showPage()
	{
		...
		} else if (this.item.ht_userdata == this.dtPg.sceneUcAudio8) {
	...
	showUcData11()
	{
		...
		console.log('showUcData8()', this.dtPg.resultUcData11);
	}
	...
	showUcData13()
	{
		...
		let result1:number = Math.abs(this.dtPg2nd.countTodayFeelRating) - Math.abs(this.dtPg2nd.countUcFeelRating12);
		let result2:number = Math.abs(this.dtPg3rd.countTodayFeelRating) - Math.abs(this.dtPg3rd.countUcFeelRating12);
		let result3:number = Math.abs(this.dtPg4th.countTodayFeelRating) - Math.abs(this.dtPg4th.countUcFeelRating12);
		let result4:number = Math.abs(this.dtPg.countTodayFeelRating) - Math.abs(this.dtPg.countUcFeelRating10);
		...
		if ((Math.abs(result1) >= Math.abs(result2)) && (Math.abs(result1) >= Math.abs(result3)) && (Math.abs(result1) >= Math.abs(result4)) )
		{
			...
			// 장면 이완 오디오
			this.dtPg.resultUcData13 = 4;
		} else {
			// 근육 이완 동영상
			this.dtPg.resultUcData13 = 5;
		}
