...
import { Ht2Pg48Data } from './ht2_pg48.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg48',
  templateUrl: 'ht2_pg48.html',
})
export class Ht2Pg48Page {

	/* Common */
	pgConfig = new Ht2PgConfig(48);
	end_zone_num = 5;
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput13) {
			...
			if (this.dtPg.resultUcInput30[1] == -1 ||
				this.dtPg.resultUcInput30[2] == -1 ||
				this.dtPg.resultUcInput30[3] == -1)
			{
				this.util.toast('보기의 항목을 모두 선택하세요.');
				return false;
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput30) {
			this.showUcInput30();
		}
	...
	selectUcInput13(num) {
		...
		alert.addButton({
			text: '선택',
			handler: data => {
				this.dtPg.resultUcInput13[num] = Number(data);
			}
		});
	...
	selectUcInput30(num) {
		...
		alert.addButton({
			text: '선택',
			handler: data => {
				this.dtPg.resultUcInput30[num] = Number(data);
			}
		});
	...
	sendUcInput30()
	{
			...
			this.dtPg.resultUcInput30[3] == -1)
		{
			this.util.toast('보기의 항목을 모두 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput30[1] == this.dtPg.passUcInput30[1] &&
			this.dtPg.resultUcInput30[2] == this.dtPg.passUcInput30[2] &&
			this.dtPg.resultUcInput30[3] == this.dtPg.passUcInput30[3])
		{
			this.util.alert('잘 하셨습니다.');
