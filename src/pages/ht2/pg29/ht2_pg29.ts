...
import { Ht2Pg29Data } from './ht2_pg29.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg29',
  templateUrl: 'ht2_pg29.html',
})
export class Ht2Pg29Page {

	/* Common */
	pgConfig = new Ht2PgConfig(29);
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg29Page');
	}
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput49) {
			...
			if (this.dtPg.resultUcInput56 == -1) {
				this.util.toast('보기의 항목을 선택하세요.');
				return false;
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcTime61) {
			if (this.dtPg.isUcTime61 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput62) {
			if (this.dtPg.resultUcInput62 == -1) {
				this.util.toast('보기의 항목을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput62 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput63) {
			if (this.dtPg.resultUcInput63[1] == false &&
				this.dtPg.resultUcInput63[2] == false &&
				this.dtPg.resultUcInput63[3] == false) {
				this.util.toast('보기의 항목을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput63 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
	...
	/* user time */
	changeAmPmUpTime61(num)
	{
		if (this.dtPg.resultUcTime61[num].ampm == '오전') {
			this.dtPg.resultUcTime61[num].ampm = '오후';
		} else {
			this.dtPg.resultUcTime61[num].ampm = '오전';
		}
	}

	changeAmPmDownTime61(num)
	{
		if (this.dtPg.resultUcTime61[num].ampm == '오전') {
			this.dtPg.resultUcTime61[num].ampm = '오후';
		} else {
	...
	changeHourUpTime61(num)
	{
		if (this.dtPg.resultUcTime61[num].hour == 12) {
			this.dtPg.resultUcTime61[num].hour = 1;
		} else {
			this.dtPg.resultUcTime61[num].hour += 1;
		}
	}

	changeHourDownTime61(num)
	{
		if (this.dtPg.resultUcTime61[num].hour == 1) {
			this.dtPg.resultUcTime61[num].hour = 12;
		} else {
			this.dtPg.resultUcTime61[num].hour -= 1;
		}
	}

	changeMinUpTime61(num)
	{
		if (this.dtPg.resultUcTime61[num].min == 55) {
			this.dtPg.resultUcTime61[num].min = 0;
		} else {
			this.dtPg.resultUcTime61[num].min += 5;
		}
	}

	changeMinDownTime61(num)
	{
		if (this.dtPg.resultUcTime61[num].min == 0) {
			this.dtPg.resultUcTime61[num].min = 55;
		} else {
			this.dtPg.resultUcTime61[num].min -= 5;
		}
	}
	...
	sendUcTime61()
	{
		console.log('sendUcTime61');

		if (this.dtPg.isUcTime61 == false) {

			this.util.toast('해당 시간을 입력하셨습니다.');
			this.dtPg.isUcTime61 = true;
