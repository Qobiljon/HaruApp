...
import { Ht2Pg10Data } from './ht2_pg10.data';

@IonicPage()
@Component({
  selector: 'ht2-page-pg10',
  templateUrl: 'ht2_pg10.html',
})
export class Ht2Pg10Page {

	/* Common */
	pgConfig = new Ht2PgConfig(10);
	...
	goPrev()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData52 && this.dtPg.selectUcInput49 == 1 ) {
			// 분기#1
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData50;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData50 && this.dtPg.selectUcInput49 == 1 ) {
			// 분기#1
			this.pgConfig.sceneNumber = this.dtPg.sceneUcInput49;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData52 && this.dtPg.selectUcInput49 == 2 ) {
			// 분기#2
			this.pgConfig.sceneNumber = this.dtPg.sceneUcData51;
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData51 && this.dtPg.selectUcInput49 == 2 ) {
			// 분기#2
			this.pgConfig.sceneNumber = this.dtPg.sceneUcInput49;
		} else {
	...
	goNext()
	{
		...
		if (this.checkInput() == true)
		{
			...
			if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput49 && this.dtPg.selectUcInput49 == 1 ) {
				// 분기#1
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData50;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData50 && this.dtPg.selectUcInput49 == 1 ) {
				// 분기#1
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData52;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput49 && this.dtPg.selectUcInput49 == 2 ) {
				// 분기#2
				this.pgConfig.sceneNumber = this.dtPg.sceneUcData51;
			} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcData51 && this.dtPg.selectUcInput49 == 2 ) {
				// 분기#2
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput16) {
			...
			if (this.dtPg.resultUcInput49[1] == false &&
				this.dtPg.resultUcInput49[2] == false &&
				this.dtPg.resultUcInput49[3] == false &&
				this.dtPg.resultUcInput49[4] == false &&
				this.dtPg.resultUcInput49[5] == false )
			{
				this.util.toast('보기의 답을 선택하세요.');
				return false;
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput16) {
			this.showUcInput16();
		} else if (this.item.ht_userdata == this.dtPg.sceneUcInput49) {
			this.showUcInput49();
	...
	showUcInput49()
	{
			...
			this.dtPg.resultUcInput49[5] == false )
		{
			...
			if (this.dtPg.resultUcInput49[5] == false)
				this.dtPg.selectUcInput49 = 1;
			else
				this.dtPg.selectUcInput49 = 2;
	...
	checkUcInput49(num)
	{
		console.log('checkUcInput49()');

		if (num == 5)
		{
			this.dtPg.resultUcInput49[1] = false;
			this.dtPg.resultUcInput49[2] = false;
			this.dtPg.resultUcInput49[3] = false;
			this.dtPg.resultUcInput49[4] = false;
		} else {
