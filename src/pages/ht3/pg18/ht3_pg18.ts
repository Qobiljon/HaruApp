...
import { Ht3Pg18Data } from './ht3_pg18.data';
import { Ht3Pg16Data } from '../pg16/ht3_pg16.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg18',
  templateUrl: 'ht3_pg18.html',
})
export class Ht3Pg18Page {

	/* Common */
	pgConfig = new Ht3PgConfig(18);
	end_zone_num = 2;
	...
	/* User Custom */
	dtPg1st = new Ht3Pg16Data();
	dtPg = new Ht3Pg18Data();
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput8) {
			if (this.dtPg.resultUcInput8[1] == -1 &&
				this.dtPg.resultUcInput8[2] == -1)
			{
	...
	sendUcInput8()
	{
			...
			this.dtPg.resultUcInput8[2] == -1 )
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput8[1] == this.dtPg.passUcInput8[1] &&
			this.dtPg.resultUcInput8[2] == this.dtPg.passUcInput8[2] )
		{
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput8 = true;
			return;
		} else {
			this.dtPg.countUcInput8--;

			if (this.dtPg.countUcInput8 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput8 = true;
				this.dtPg.resultUcInput8 = this.dtPg.passUcInput8;
			}
