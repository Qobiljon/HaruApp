...
import { Ht3Pg45Data } from './ht3_pg45.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg45',
  templateUrl: 'ht3_pg45.html',
})
export class Ht3Pg45Page {

	/* Common */
	pgConfig = new Ht3PgConfig(45);
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg45Page');
	}
	...
	sendUcInput13()
	{
			...
			this.dtPg.resultUcInput13[4] == false )
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput13[1] == this.dtPg.passUcInput13[1] &&
			this.dtPg.resultUcInput13[2] == this.dtPg.passUcInput13[2] &&
			this.dtPg.resultUcInput13[3] == this.dtPg.passUcInput13[3] &&
			this.dtPg.resultUcInput13[4] == this.dtPg.passUcInput13[4] )
		{
	...
	sendUcInput22()
	{
			...
			this.dtPg.resultUcInput22[4] == false )
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput22[1] == this.dtPg.passUcInput22[1] &&
			this.dtPg.resultUcInput22[2] == this.dtPg.passUcInput22[2] &&
			this.dtPg.resultUcInput22[3] == this.dtPg.passUcInput22[3] &&
			this.dtPg.resultUcInput22[4] == this.dtPg.passUcInput22[4] )
		{
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput22 = true;
		} else {
			this.dtPg.countUcInput22--;

			if (this.dtPg.countUcInput22 >= 0) {
				this.util.alert('다시 한 번 생각해보세요.');
			} else {
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput22 = true;
				this.dtPg.resultUcInput22 = this.dtPg.passUcInput22;
			}
