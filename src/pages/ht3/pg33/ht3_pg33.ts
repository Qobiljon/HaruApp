...
import { Ht3Pg33Data } from './ht3_pg33.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg33',
  templateUrl: 'ht3_pg33.html',
})
export class Ht3Pg33Page {

	/* Common */
	pgConfig = new Ht3PgConfig(33);
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg33Page');
	}
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating16) {
			if (this.dtPg.resultUcFeelRating16 == -1) {
			...
			if (this.dtPg.resultUcInput27[1] == -1 ||
				this.dtPg.resultUcInput27[2] == -1 ||
				this.dtPg.resultUcInput27[3] == -1 ||
				this.dtPg.resultUcInput27[4] == -1)
			{
			...
			if (this.dtPg.isUcInput31 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput32) {
			if (this.dtPg.isUcInput32 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput33) {
			if (this.dtPg.isUcInput33 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating39) {
			if (this.dtPg.resultUcFeelRating39 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating39 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput31) {
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput32) {
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput33) {
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput34) {
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcData35) {
	...
	sendUcFeelRating16()
	{
		console.log("sendUcFeelRating16", this.dtPg.resultUcFeelRating16);
	...
	sendUcInput18()
	{
		console.log("sendUcInput18", this.dtPg.countUcInput18);
	...
	sendUcOrder27(num)
	{
		console.log('sendUcOrder27()');

		for (let step:number = 1; step <=4; step++) {
			if (this.dtPg.orderUcInput27[step] == -1)
			{
				this.dtPg.orderUcInput27[step] = num;
				this.dtPg.resultUcInput27[num] = step;
				break;
			}
		}
	}
	...
	removeUcOrder27(num)
	{
		console.log('removeUcOrder27()');

		for (let step:number = 1; step <=4; step++) {
			if (this.dtPg.orderUcInput27[step] == num)
			{
				this.dtPg.orderUcInput27[step] = -1;
				this.dtPg.resultUcInput27[num] = -1;
				break;
			}
		}
	}
	...
	changeUp(num) {
		...
		if (this.dtPg.sumUcInput31 < 100)
		{
			if (num == 1) {
				this.dtPg.resultUcInput31 += 5;
			} else if (num == 2) {
				this.dtPg.resultUcInput32 += 5;
			} else if (num == 3) {
				this.dtPg.resultUcInput33 += 5;
			}

			this.dtPg.sumUcInput31 = this.dtPg.resultUcInput31;
			this.dtPg.sumUcInput31 += this.dtPg.resultUcInput32;
			this.dtPg.sumUcInput31 += this.dtPg.resultUcInput33;

			this.dtPg.resultUcInput34 = 100 - this.dtPg.sumUcInput31;
		} else {
	...
	sendUcFeelRating39()
	{
		console.log("sendUcFeelRating39", this.dtPg.resultUcFeelRating39);
		...
		if (this.dtPg.resultUcFeelRating39 == -1)
		{
			...
			this.util.toast('기분평점을 평가하셨습니다.');
			this.dtPg.isUcFeelRating39 = true;
			return;		
		}
	}
	...
	drawPieChart1()
	{
		...
		this.objPieChart1 = new Chart(this.pieCanvas.nativeElement, {
			type: 'pie',
			data: {
				labels: ['1. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[1]] + ' (' + String(this.dtPg.resultUcInput31) + '%)',
						''],
				datasets: [{
					label: '파이차트',
					data: [this.dtPg.resultUcInput31, this.dtPg.resultUcInput34],
					backgroundColor: [
						'rgba(255, 99, 132, 0.7)',
	...
	drawPieChart2()
	{
		...
		this.objPieChart2 = new Chart(this.pieCanvas.nativeElement, {
			...
			data: {
				labels: ['1. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[1]] + ' (' + String(this.dtPg.resultUcInput31) + '%)',
						'2. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[2]] + ' (' + String(this.dtPg.resultUcInput32) + '%)',
						''],
				datasets: [{
					label: '파이차트',
					data: [this.dtPg.resultUcInput31, this.dtPg.resultUcInput32, this.dtPg.resultUcInput34],
					backgroundColor: [
						'rgba(255, 99, 132, 0.7)',
						'rgba(54, 162, 235, 0.7)',
						'rgba(255, 255, 255, 1.0)'
	...
	drawPieChart3()
	{
		...
		this.objPieChart3 = new Chart(this.pieCanvas.nativeElement, {
			...
			data: {
				labels: ['1. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[1]] + ' (' + String(this.dtPg.resultUcInput31) + '%)',
						'2. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[2]] + ' (' + String(this.dtPg.resultUcInput32) + '%)',
						'3. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[3]] + ' (' + String(this.dtPg.resultUcInput33) + '%)',
						''],
				datasets: [{
					label: '파이차트',
					data: [this.dtPg.resultUcInput31, this.dtPg.resultUcInput32, this.dtPg.resultUcInput33, this.dtPg.resultUcInput34],
					backgroundColor: [
						'rgba(255, 99, 132, 0.7)',
						'rgba(54, 162, 235, 0.7)',
						'rgba(255, 206, 86, 0.7)',
		...
		if (this.item.ht_tpcode == 'hcst033') {
			setTimeout(() => {
	...
	drawPieChart4()
	{
		...
		this.objPieChart4 = new Chart(this.pieCanvas.nativeElement, {
			...
			data: {
				labels: ['1. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[1]] + ' (' + String(this.dtPg.resultUcInput31) + '%)',
						'2. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[2]] + ' (' + String(this.dtPg.resultUcInput32) + '%)',
						'3. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[3]] + ' (' + String(this.dtPg.resultUcInput33) + '%)',
						'4. ' + this.dtPg.textUcInput27[this.dtPg.orderUcInput27[4]] + ' (' + String(this.dtPg.resultUcInput34) + '%)'],
				datasets: [{
					label: '파이차트',
					data: [this.dtPg.resultUcInput31, this.dtPg.resultUcInput32, this.dtPg.resultUcInput33, this.dtPg.resultUcInput34],
					backgroundColor: [
		...
		if (this.item.ht_tpcode == 'hcst034') {
			setTimeout(() => {
	...
	drawPieChart5()
	{
		...
		if (this.item.ht_tpcode == 'hcst035') {
			setTimeout(() => {
