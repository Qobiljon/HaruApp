...
import { Ht3Pg35Data } from './ht3_pg35.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg35',
  templateUrl: 'ht3_pg35.html',
})
export class Ht3Pg35Page {

	/* Common */
	pgConfig = new Ht3PgConfig(35);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating19) {
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating33) {
			if (this.dtPg.resultUcFeelRating33 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating33 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput35) {
			if (this.dtPg.countUcInput35 == -1) {
	...
	sendUcFeelRating33()
	{
		console.log("sendUcFeelRating33", this.dtPg.resultUcFeelRating33);
		...
		if (this.dtPg.resultUcFeelRating33 == -1)
		{
			...
			this.util.toast('기분평점을 평가하셨습니다.');
			this.dtPg.isUcFeelRating33 = true;
			return;		
		}
	}
	...
	sendUcInput35()
	{
		console.log("sendUcInput35", this.dtPg.countUcInput35);
