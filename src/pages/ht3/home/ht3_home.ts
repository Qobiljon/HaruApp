...
@IonicPage()
@Component({
  selector: 'ht3-page-home',
  templateUrl: 'ht3_home.html'
})
export class Ht3HomePage {
	/* Common */
	pgConfig = new Ht3PgConfig(0);
		...
		public backgroundMode: BackgroundMode)
	{
		...
		this.events.subscribe('postHaruTodayInfo:changed', (read, testDateText, title, ratingJoined, ratingPoint, totalPoint) => {
		...
		this.events.subscribe('eventHaruTodayBookmark:changed', (num, title, bookmark) => {
		...
		this.events.subscribe('alarmInfo:changed', (alarmUse, alarmMode, alarmHour, alarmMin) => {
		...
		this.events.subscribe('lastRatingDate:changed', (data) => {
	...
	setAlarmSound() {
		if (this.platform.is('android')) {
			return 'file://assets/ht3/audio/alarm.mp3'
		} else {
			return 'file://assets/ht3/audio/alarm.caf'
		}
	}
	...
	postHaruTodayInfo()
	{
		...
		.then((data) => {
			...
			if (responseData.mode == 'harutoday_info' && responseData.code == 'ok')
			{
				...
				this.storage.eventPostHaruTodayInfo(haruTodayReadList[this.todayCount], this.testDateText, title, this.ratingJoined, this.ratingPoint, this.totalPoint);
			}
		}, (err) => {
			// Error log
	...
	postHaruTodayMenu()
	{
		...
		let pgConfig = new Ht3PgConfig(0);
	...
	postHaruTodayLoadUserData(todayCount)
	{
		...
		let pgConfig = new Ht3PgConfig(todayCount);
	...
	openHistory()
	{
		let historyModal = this.modalCtrl.create('Ht3HistoryPage');
	...
	openRatingHistory()
	{
		let ratingHistoryModal = this.modalCtrl.create('Ht3RatingHistoryPage');
	...
	openRatingGraph()
	{
		let ratingGraphModal = this.modalCtrl.create('Ht3RatingGraphPage');
	...
	openGift(totalPoint)
	{
		...
		if ( giftItem !=null && giftItem.activated == 1)
		{
			...
			if (selectCount > 0 ) {
				// 기프티콘 선택 팝업
				let giftModal = this.modalCtrl.create('Ht3GiftPage');
