...
import { Ht3Pg19Data } from './ht3_pg19.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg19',
  templateUrl: 'ht3_pg19.html',
})
export class Ht3Pg19Page {

	/* Common */
	pgConfig = new Ht3PgConfig(19);
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg19Page');
	}
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput31) {
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput35) {
			if (this.dtPg.countUcInput35 != 3) {
				this.util.toast('가장 빈번한 것을 3개 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput35 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput36) {
			if (this.dtPg.countUcInput36 == -1) {
				this.util.toast('통증이 얼마나 심한지 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput36 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput37) {
			if (this.dtPg.resultUcInput37[1] == -1 || this.dtPg.resultUcInput37[2] == -1) {
				this.util.toast('보기의 항목을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput37 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput38) {
			if (this.dtPg.resultUcInput38[1] == -1 || this.dtPg.resultUcInput38[2] == -1) {
				this.util.toast('보기의 항목을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput38 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
	...
	checkUcInput34(val)
	{
		this.dtPg.countUcInput34 = 0;

		for (let i = 1; i <= 14; i++) {
			if (this.dtPg.resultUcInput34[i])
				this.dtPg.countUcInput34++;
		}

		this.dtPg.selectUcInput34[1] = -1;
		this.dtPg.selectUcInput34[2] = -1;
		this.dtPg.selectUcInput34[3] = -1;
		...
		if (this.dtPg.countUcInput34 > 3) {
			...
			for (let i = 1; i <= 14; i++) {
				if (this.dtPg.resultUcInput34[i]) {
					this.dtPg.selectUcInput34[1] = i;
					break;
				}
			}

			for (let i = 1; i <= 14; i++) {
				if (this.dtPg.resultUcInput34[i] && this.dtPg.selectUcInput34[1] != i) {
					this.dtPg.selectUcInput34[2] = i;
					break;
				}
			}

			for (let i = 1; i <= 14; i++) {
				if (this.dtPg.resultUcInput34[i] && this.dtPg.selectUcInput34[1] != i && this.dtPg.selectUcInput34[2] != i) {
					this.dtPg.selectUcInput34[3] = i;
					break;
				}
			}
		}
	...
	checkUcInput35(val)
	{
		this.dtPg.countUcInput35 = 0;

		for (let i = 1; i <= 13; i++) {
			if (this.dtPg.resultUcInput35[i])
				this.dtPg.countUcInput35++;
		}

		this.dtPg.selectUcInput35[1] = -1;
		this.dtPg.selectUcInput35[2] = -1;
		this.dtPg.selectUcInput35[3] = -1;

		if (this.dtPg.countUcInput35 > 3) {
			this.util.alert('3개를 초과할 수 없습니다.');
			val.checked = false;
		} else {

			for (let i = 1; i <= 13; i++) {
				if (this.dtPg.resultUcInput35[i]) {
					this.dtPg.selectUcInput35[1] = i;
					break;
				}
			}

			for (let i = 1; i <= 13; i++) {
				if (this.dtPg.resultUcInput35[i] && this.dtPg.selectUcInput35[1] != i) {
					this.dtPg.selectUcInput35[2] = i;
					break;
				}
			}

			for (let i = 1; i <= 13; i++) {
				if (this.dtPg.resultUcInput35[i] && this.dtPg.selectUcInput35[1] != i && this.dtPg.selectUcInput35[2] != i) {
					this.dtPg.selectUcInput35[3] = i;
					break;
				}
			}
		}
	...
	sendTodayRead()
	{
		...
		let requestData = {"mode": "harutoday_read", "id": userInfo.id, "key": userInfo.key, "code": this.itemTarget.mcode, "num": this.itemTarget.pnum, "quizpoint": this.pgConfig.pointQuiz, "zone_num": 3};
