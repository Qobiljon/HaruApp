...
import { Ht3Pg34Data } from './ht3_pg34.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg34',
  templateUrl: 'ht3_pg34.html',
})
export class Ht3Pg34Page {

	/* Common */
	pgConfig = new Ht3PgConfig(34);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput41) {
	...
	sendUcInput41()
	{
		...
		if (this.dtPg.resultUcInput41 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput41 == this.dtPg.passUcInput41) {
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput41 = true;
		} else {

			this.dtPg.countUcInput41--;

			if (this.dtPg.countUcInput41 >= 0) {
				this.util.alert('다시 한 번 생각해보세요. 다시 한번 생각해 보세요.');
			} else {
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput41 = true;
				this.dtPg.resultUcInput41 = this.dtPg.passUcInput41;
			}
	...
	sendUcInput43()
	{
		...
		if (this.dtPg.resultUcInput43 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput43 == this.dtPg.passUcInput43) {
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput43 = true;
		} else {

			this.dtPg.countUcInput43--;

			if (this.dtPg.countUcInput43 >= 0) {
				this.util.alert('다시 한 번 생각해보세요. 다시 한번 생각해 보세요.');
			} else {
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput43 = true;
				this.dtPg.resultUcInput43 = this.dtPg.passUcInput43;
			}
