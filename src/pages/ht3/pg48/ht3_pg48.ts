...
import { Ht3Pg48Data } from './ht3_pg48.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg48',
  templateUrl: 'ht3_pg48.html',
})
export class Ht3Pg48Page {

	/* Common */
	pgConfig = new Ht3PgConfig(48);
	end_zone_num = 5;
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcInput28) {
			this.showUcInput28();
		}
	...
	selectUcInput11(num) {
		...
		alert.addButton({
			text: '선택',
			handler: data => {
				this.dtPg.resultUcInput11[num] = Number(data);
			}
		});
	...
	sendUcInput11()
	{
			...
			this.dtPg.resultUcInput11[3] == -1)
		{
			this.util.toast('보기의 항목을 모두 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput11[1] == this.dtPg.passUcInput11[1] &&
			this.dtPg.resultUcInput11[2] == this.dtPg.passUcInput11[2] &&
			this.dtPg.resultUcInput11[3] == this.dtPg.passUcInput11[3])
		{
