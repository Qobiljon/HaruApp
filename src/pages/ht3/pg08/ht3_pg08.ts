...
import { Ht3Pg08Data } from './ht3_pg08.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg08',
  templateUrl: 'ht3_pg08.html',
})
export class Ht3Pg08Page {

	/* Common */
	pgConfig = new Ht3PgConfig(8);
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad Pg08Page');
	}
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcAlarm26) {
			if (this.dtPg.isUcAlarm26 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
	...
	showPage()
	{
		...
		if (this.item.ht_userdata == this.dtPg.sceneUcAlarm26) {
			this.showUcAlarm26();
	...
	sendUcAlarm26()
	{
		console.log("sendUcAlarm26");

		if (this.dtPg.isUcAlarm26 == false) {
			this.util.toast('훈련 시간을 입력하셨습니다.');
			this.dtPg.isUcAlarm26 = true;

			console.log("dtPg.resultUcAlarm26.ampm", this.dtPg.resultUcAlarm26.ampm);
			console.log("dtPg.resultUcAlarm26.hour", this.dtPg.resultUcAlarm26.hour);
	...
	sendUcInput17()
	{
			...
			this.dtPg.resultUcInput17[6] == false )
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput17[1] == this.dtPg.passUcInput17[1] &&
			this.dtPg.resultUcInput17[2] == this.dtPg.passUcInput17[2] &&
			this.dtPg.resultUcInput17[3] == this.dtPg.passUcInput17[3] &&
			this.dtPg.resultUcInput17[4] == this.dtPg.passUcInput17[4] &&
			this.dtPg.resultUcInput17[5] == this.dtPg.passUcInput17[5] &&
			this.dtPg.resultUcInput17[6] == this.dtPg.passUcInput17[6] )
		{
	...
	changeUcAmPmUp()
	{
		if (this.dtPg.resultUcAlarm26.ampm == '오전') {
			this.dtPg.resultUcAlarm26.ampm = '오후';
		} else {
			this.dtPg.resultUcAlarm26.ampm = '오전';
		}
	}
	...
	changeUcHourUp()
	{
		if (this.dtPg.resultUcAlarm26.hour == 12) {
			this.dtPg.resultUcAlarm26.hour = 1;
		} else {
			this.dtPg.resultUcAlarm26.hour += 1;
		}
	}
	...
	changeUcHourDown()
	{
		if (this.dtPg.resultUcAlarm26.hour == 1) {
			this.dtPg.resultUcAlarm26.hour = 12;
		} else {
			this.dtPg.resultUcAlarm26.hour -= 1;
		}
	}
	...
	changeUcMinUp()
	{
		if (this.dtPg.resultUcAlarm26.min == 55) {
			this.dtPg.resultUcAlarm26.min = 0;
		} else {
			this.dtPg.resultUcAlarm26.min += 5;
		}
	}

	changeUcMinDown()
	{
		if (this.dtPg.resultUcAlarm26.min == 0) {
			this.dtPg.resultUcAlarm26.min = 55;
		} else {
			this.dtPg.resultUcAlarm26.min -= 5;
		}
	}
