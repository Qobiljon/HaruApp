...
import { Ht3Pg29Data } from './ht3_pg29.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg29',
  templateUrl: 'ht3_pg29.html',
})
export class Ht3Pg29Page {

	/* Common */
	pgConfig = new Ht3PgConfig(29);
	...
	checkInput()
	{
		...
		if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput13) {
			...
			if (this.dtPg.resultUcInput15 == -1)
			{
				this.util.toast('보기의 답을 선택하세요.');
				return false;
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating16) {
	...
	sendUcInput15()
	{
		...
		if (this.dtPg.resultUcInput15 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput15 == this.dtPg.passUcInput15) {
			this.util.alert('잘 하셨습니다.');
