...
import { Ht3Pg37Data } from './ht3_pg37.data';

@IonicPage()
@Component({
  selector: 'ht3-page-pg37',
  templateUrl: 'ht3_pg37.html',
})
export class Ht3Pg37Page {

	/* Common */
	pgConfig = new Ht3PgConfig(37);
	...
	checkInput()
	{
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating28) {
			if (this.dtPg.resultUcFeelRating28 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating28 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		...
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput60) {
			if (this.dtPg.resultUcInput60 == -1)
			{
				this.util.toast('보기의 답을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput60 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcInput66) {
			if (this.dtPg.countUcInput66 == -1) {
				this.util.toast('이때의 점수를 선택하세요.');
				return false;
			} else if (this.dtPg.isUcInput66 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		} else if (this.pgConfig.sceneNumber == this.dtPg.sceneUcFeelRating67) {
			if (this.dtPg.resultUcFeelRating67 == -1) {
				this.util.toast('이때의 기분평점을 선택하세요.');
				return false;
			} else if (this.dtPg.isUcFeelRating67 == false) {
				this.util.alert('제출하기 버튼을 선택하세요.');
				return false;
			}
		}
	...
	sendUcInput27()
	{
		console.log("sendUcInput27", this.dtPg.countUcInput27);
	...
	sendUcFeelRating28()
	{
		console.log("sendUcFeelRating28", this.dtPg.resultUcFeelRating28);
		...
		if (this.dtPg.resultUcFeelRating28 == -1)
		{
			...
			this.dtPg.isUcFeelRating28 = true;
			return;		
		}
	}
	...
	sendUcInput60()
	{
		console.log('sendUcInput60()');

		if (this.dtPg.resultUcInput60 == -1)
		{
			this.util.toast('보기의 답을 선택하세요.');
			return;
		} else if (this.dtPg.resultUcInput60 == this.dtPg.passUcInput60) {
			this.util.alert('잘 하셨습니다.');
			this.dtPg.isUcInput60 = true;
		} else {

			this.dtPg.countUcInput60--;

			if (this.dtPg.countUcInput60 >= 0) {
				this.util.alert('다시 한 번 생각해보세요. 다시 한번 생각해 보세요.');
			} else {
				this.util.alert('다시 한 번 생각해보세요. 응답 횟수를 초과하였습니다.');
				this.dtPg.isUcInput60 = true;
				this.dtPg.resultUcInput60 = this.dtPg.passUcInput60;
			}
	...
	sendUcInput66()
	{
		console.log("sendUcInput66", this.dtPg.countUcInput66);
	...
	sendUcFeelRating67()
	{
		console.log("sendUcFeelRating67", this.dtPg.resultUcFeelRating67);
		...
		if (this.dtPg.resultUcFeelRating67 == -1)
		{
			...
			this.util.toast('기분평점을 평가하셨습니다.');
			this.dtPg.isUcFeelRating67 = true;
			return;		
		}
	}
